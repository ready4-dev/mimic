...
#' @param what_1L_chr What (a character vector of length one), Default: c("arm", "event")
...
methods::setMethod("procure", "MimicConfiguration", function (x, empty_xx = NULL, match_value_xx = NULL, target_1L_chr = character(0), 
    type_1L_chr = "Arm", what_1L_chr = c("arm", "event")) 
{
            ...
            if (!identical(target_1L_chr, character(0))) {
                if (target_1L_chr %in% names(object_xx)) {
                  object_xx <- get_from_lup_obj(object_xx, target_var_nm_1L_chr = target_1L_chr, 
                    match_var_nm_1L_chr = type_1L_chr, match_value_xx = match_value_xx)[[1]]
                }
                else {
                  object_xx <- empty_xx
                }
    ...
    if (what_1L_chr == "event") {
        ...
        if (!identical(object_xx, list()) & !identical(target_1L_chr, 
            character(0))) {
            if (target_1L_chr %in% names(object_xx)) {

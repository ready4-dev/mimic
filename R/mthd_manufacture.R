...
#' @param what_1L_chr What (a character vector of length one), Default: 'args_ls'
#' @param ... Additional arguments
#' @return Object (an output object of multiple potential types)
#' @rdname manufacture-methods
...
#' @importFrom ready4 manufacture
methods::setMethod("manufacture", "MimicEligible", function (x, append_ls = list(), type_1L_chr = c("filter", "reset"), 
    what_1L_chr = "args_ls", ...) 
{
    ...
    if (what_1L_chr == "args_ls") {
        object_xx <- list(ineligible_1L_chr = x@ineligible_1L_chr, 
            post_fn = x@functions_ls$post_fn, pre_fn = x@functions_ls$pre_fn, 
            type_1L_chr = type_1L_chr)
        if (!is.null(names(append_ls))) {
            object_xx <- object_xx %>% purrr::discard_at(names(append_ls)) %>% 
                append(append_ls)
        }
    }
    return(object_xx)
...
#' @param include_chr Include (a character vector), Default: 'Modelled'
#' @param subset_1L_chr Subset (a character vector of length one), Default: character(0)
#' @param target_1L_chr Target (a character vector of length one), Default: character(0)
#' @param total_1L_lgl Total (a logical vector of length one), Default: TRUE
#' @param type_1L_chr Type (a character vector of length one), Default: c("current", "concept", "entry", "measure")
#' @param what_1L_chr What (a character vector of length one), Default: c("draws_tb", "append_ls", "args_all", "daystonever", "iterations", 
...
methods::setMethod("manufacture", "MimicConfiguration", function (x, arm_1L_chr = NA_character_, batch_1L_int = integer(0), 
    draws_tb = NULL, extras_ls = list(), include_chr = "Modelled", 
    subset_1L_chr = character(0), target_1L_chr = character(0), 
    total_1L_lgl = TRUE, type_1L_chr = c("current", "concept", 
        "entry", "measure"), what_1L_chr = c("draws_tb", "append_ls", 
        "args_all", "daystonever", "iterations", "modifiable", 
        "outcomes", "population_ls", "resources", "utilities"), 
    ...) 
{
    type_1L_chr <- match.arg(type_1L_chr)
    what_1L_chr <- match.arg(what_1L_chr)
    ...
    if (what_1L_chr == "append_ls") {
        object_xx <- manufacture(x@x_MimicArguments, batch_1L_int = batch_1L_int, 
            env_ls = make_sim_env_ls(manufacture(x, arm_1L_chr = arm_1L_chr, 
                batch_1L_int = batch_1L_int, draws_tb = draws_tb, 
                extras_ls = extras_ls, what_1L_chr = "args_all"), 
                discard_chr = "X_MimicConfiguration"), what_1L_chr = "args_ls", 
    ...
    if (what_1L_chr == "daystonever") {
        object_xx <- floor(x@horizon_dtm/lubridate::days(1)) + 
            1
    }
    ...
    if (what_1L_chr %in% c("modifiable", "outcomes", "resources", 
        "utilities")) {
        object_xx <- c(character(0))
        if (what_1L_chr %in% c("modifiable", "outcomes")) {
            object_xx <- c(object_xx, manufacture(x@x_MimicVariables, 
                include_chr = include_chr, subset_1L_chr = subset_1L_chr, 
                target_1L_chr = target_1L_chr, total_1L_lgl = total_1L_lgl, 
                type_1L_chr = type_1L_chr, what_1L_chr = "outcomes"))
        }
        ...
        if (what_1L_chr %in% c("modifiable", "resources")) {
            object_xx <- c(object_xx, manufacture(x@x_MimicVariables, 
                include_chr = include_chr, subset_1L_chr = subset_1L_chr, 
                target_1L_chr = target_1L_chr, total_1L_lgl = total_1L_lgl, 
                type_1L_chr = type_1L_chr, what_1L_chr = "resources"))
        }
    ...
    if (what_1L_chr == "population_ls") {
        ...
        if (type_1L_chr == "entry") {
            object_xx <- add_enter_model_event(X_Ready4useDyad = x@x_MimicInputs@y_Ready4useDyad, 
                arm_1L_chr = arm_1L_chr, default_fn = x@x_MimicAlgorithms@processing_ls$initialise_ls$default_fn, 
                default_args_ls = list(sensitivities_ls = x@x_MimicAlgorithms@sensitivities_ls, 
                  X_MimicVariable = x@x_MimicVariables), derive_fn_ls = x@x_MimicAlgorithms@processing_ls$initialise_ls$derive_ls, 
                draws_tb = draws_tb, horizon_dtm = x@horizon_dtm, 
                iterations_int = manufacture(x, batch_1L_int = batch_1L_int, 
                  what_1L_chr = "iterations"), modifiable_chr = c(manufacture(x, 
                  include_chr = include_chr, target_1L_chr = target_1L_chr, 
                  total_1L_lgl = total_1L_lgl, type_1L_chr = "measure", 
                  what_1L_chr = "outcomes"), manufacture(x, what_1L_chr = "utilities")), 
                ...
                tidy_cols_1L_lgl = T, tx_prefix_1L_chr = x@tx_prefix_1L_chr) %>% 
...
#' @param subset_1L_chr Subset (a character vector of length one), Default: character(0)
#' @param target_1L_chr Target (a character vector of length one), Default: character(0)
#' @param total_1L_lgl Total (a logical vector of length one), Default: FALSE
...
#' @export 
#' @importFrom dplyr filter pull mutate group_by reframe select
#' @importFrom purrr map2_chr flatten_chr discard
#' @importFrom ready4 manufacture
methods::setMethod("manufacture", "MimicVariables", function (x, include_chr = character(0), subset_1L_chr = character(0), 
    target_1L_chr = character(0), total_1L_lgl = FALSE, type_1L_chr = c("measure", 
        "concept"), what_1L_chr = c("both", "outcomes", "resources"), 
    ...) 
{
    type_1L_chr <- match.arg(type_1L_chr)
    what_1L_chr <- match.arg(what_1L_chr)
    ...
    if (what_1L_chr %in% c("outcomes", "resources")) {
        if (!identical(target_1L_chr, character(0))) {
            object_xx <- object_xx %>% dplyr::filter(Category == 
                target_1L_chr)
        }
        if (!identical(subset_1L_chr, character(0))) {
            object_xx <- object_xx %>% dplyr::filter(Subcategory == 
                subset_1L_chr)
        }
        if (!identical(include_chr, character(0))) {
            object_xx <- object_xx %>% dplyr::filter(Derivation %in% 
                include_chr)
        }
        if (what_1L_chr == "outcomes") {
        ...
        if (what_1L_chr == "resources") {
            if (type_1L_chr == "concept") {
                object_xx <- object_xx %>% dplyr::mutate(Measure = "")
            }
            object_xx <- object_xx %>% dplyr::mutate(ResourceUse = paste0(Resource, 
                Measure)) %>% dplyr::group_by(Category) %>% dplyr::reframe(ResourceUse = ResourceUse, 
                Totals = purrr::map2_chr(Total, Measure, ~ifelse(is.na(.x) | 
                  (!total_1L_lgl), NA_character_, paste0(.x, 
                  .y)))) %>% dplyr::select(-Category) %>% purrr::flatten_chr() %>% 
                purrr::discard(is.na) %>% unique()
        }
    }
    if (what_1L_chr == "both") {
        object_xx <- c(character(0), manufacture(x, include_chr = include_chr, 
            subset_1L_chr = subset_1L_chr, target_1L_chr = intersect(target_1L_chr, 
                x@outcomes_tb$Category), total_1L_lgl = total_1L_lgl, 
            type_1L_chr = type_1L_chr, what_1L_chr = "outcomes"), 
            manufacture(x, include_chr = include_chr, subset_1L_chr = subset_1L_chr, 
                target_1L_chr = intersect(target_1L_chr, x@resources_tb$Category), 
                total_1L_lgl = total_1L_lgl, type_1L_chr = type_1L_chr, 
                what_1L_chr = "resources"))
    }
    return(object_xx)
})

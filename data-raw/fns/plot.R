plot_economic_results <- function(economic_results_ls,
                                  what_1L_chr,
                                  alpha_1L_dbl = 0.8,
                                  colour_1L_chr = ready4use::get_colour_codes(2)[2],
  ...
  if(type_1L_chr == "ceac"){
    plot_plt <- BCEA::ceac.plot(data_xx, graph = "ggplot2", currency = "$",
                                line = list(color = colour_1L_chr), title = title_1L_chr, ...) 
    
  }
  if(type_1L_chr == "cep"){
    plot_plt <- BCEA::ceplane.plot(data_xx, graph = "ggplot2", wtp = threshold_1L_dbl, currency = "$", point = list(color=colour_1L_chr, size=size_1L_dbl, alpha = alpha_1L_dbl), title = title_1L_chr, ...)
  }
  if(type_1L_chr == "evi"){
    plot_plt <- BCEA::evi.plot(data_xx, graph = "ggplot2", line_colors = colour_1L_chr, ...) + ggplot2::labs(title= title_1L_chr)
  }
...
plot_regression <- function(regressions_ls,
                            what_1L_chr,
                            close_1L_lgl = TRUE,
                            colors_chr = ready4use::get_colour_codes(),
...
plot_test_scatter <- function (X_Ready4useDyad, collapse_1L_lgl = F, grouping_1L_chr = character(0), new_1L_lgl = F,
  ...
  if (collapse_1L_lgl) {
    if(!new_1L_lgl){
      X_Ready4useDyad@ds_tb <- rbind(X_Ready4useDyad@ds_tb %>%
        ...
        dplyr::select(-!!rlang::sym(paste0(var_1L_chr, "_sim_mean"))) %>%
        dplyr::distinct() 
    }else{
      X_Ready4useDyad <- rbind(X_Ready4useDyad@ds_tb %>%
                                 dplyr::filter(Data != type_1L_chr), 
                               X_Ready4useDyad@ds_tb %>%
                                 dplyr::filter(Data == type_1L_chr) %>% dplyr::group_by(UID) %>% dplyr::summarise(`:=`(!!rlang::sym(var_1L_chr),
